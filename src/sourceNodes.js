...
const sourceNodes = async (
  ...
  const { getNodesByType, getNodes, reporter, loadNodeContent } = apiHelpers;
  ...
  const wpDataActivity = reporter.activityTimer(`create Wordsby node types from ./wordsby/data`);
  ...
  await Promise.all(
    wordsbyData.map( async ( node, index ) => {
        wpDataActivity.setStatus(`[ ${ index + 1 } / ${ wordsbyData.length } ]`);
  ...
  const wpRelationalDataActivity 
  ...
  const wordsbyRelationalData = wordsbyFileNodes.filter(
    ({ internal }) => internal.description.includes(`File "wordsby/data/collections`)
  )

  const availableCollectionsIds = await Promise.all(
    wordsbyRelationalData.map( async node => {
